...
function buildSubmitBody(
  ...
  if (kind === 'video' && imageUrl) {
    ...
    body.input_image = imageUrl
    body.reference_image = imageUrl
    body.reference_image_url = imageUrl
  }
...
export async function generateMediaUrl(
  ...
  const submitBody = buildSubmitBody(kind, prompt, imageUrl)

  console.log(
    '[higgsfield] submit payload:',
    JSON.stringify(
      {
        ...submitBody,
        prompt,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.log(
      '[higgsfield] final status response:',
